...
def biomedrxivsearch(
    ...
    ## keyword selection
    if len(kwd) > 0:
        kwd_str = "text_abstract_title%3A" + ("%252C%2B").join(
    ...
    url += (
        "%20numresults%3A"
        + str(num_page_results)
        + "%20format_result%3Acondensed"
    ...
    while True:
        ...
        print(
            "Fetching search results {:d} to {:d}...".format(
                num_page_results * page + 1, num_page_results * (page + 1)
            )
        )
        ...
        if page == 0:
            ...
            num_results_text = (
                html.find("div", attrs={"class": "highwire-search-summary"})
                .text.strip()
                .split()[0]
            )
        ...
        titles += [
            article.find("span", attrs={"class": "highwire-cite-title"}).text.strip()
            if article.find("span", attrs={"class": "highwire-cite-title"}) is not None
            else None
        ...
        author_lists += [
            [
                author.text
                for author in article.find_all(
        ...
        urls = [
            "http://www.{:s}.org".format(journal) + article.find("a", href=True)["href"]
            for article in articles
        ]
        ...
        dates += [
            ("-").join(
                article.find(
                    "div",
                    attrs={
                        "class": "highwire-article-citation highwire-citation-type-highwire-article"
                    },
                )
                .get("data-pisa")
                .strip()
        ...
        if (
            time.time() - overall_time > max_time
            or (page + 1) * num_page_results >= num_fetch_results
        ):
            break
    ...
    records_data = list(
        zip(
            *list(
                map(
                    lambda dummy_list: dummy_list[0:num_fetch_results],
                    [titles, author_lists, urls, dates],
                )
            )
        )
    )
    full_records_df = pd.DataFrame(
        records_data, columns=["title", "authors", "url", "date"]
    )

    ## keep user informed on why task ended
    if num_results > max_records:
        print(
            "Max number of records ({:d}) reached. Fetched in {:.1f} seconds.".format(
                max_records, time.time() - overall_time
            )
        )
    elif time.time() - overall_time > max_time:
        print(
            "Max time ({:.0f} seconds) reached. Fetched {:d} records in {:.1f} seconds.".format(
                max_time, num_fetch_results, time.time() - overall_time
            )
        )
    else:
        print(
            "Fetched {:d} records in {:.1f} seconds.".format(
                num_fetch_results, time.time() - overall_time
            )
        )
    ...
    if abstracts:
        ...
        full_records_df["abstract"] = [
            bs(requests.post(paper_url).text, features="html.parser")
            .find("div", attrs={"class": "section abstract"})
    ...
    if download != "":
        ...
        pdf_urls = [
            url + ".full.pdf" for url in full_records_df.url
        ]  # list of urls to pull pdfs from

        # create filenames to export pdfs to
        # currently setup in year_lastname format
        pdf_lastnames_full = [
            "_".join([name.split()[-1] for name in namelist])
            for namelist in full_records_df.authors
        ]  # pull out lastnames only
        pdf_lastnames = [
            name if len(name) < 200 else name.split("_")[0] + "_et_al"
            for name in pdf_lastnames_full
        ]  # make sure file names don't get longer than ~200 chars
        pdf_paths = [
            download + date + "_" + lastname + ".pdf"
            for date, lastname in zip(full_records_df.date, pdf_lastnames)
    ...
    if export != "":
        if exportfile == "":
            exportfile = datetime.date.today().strftime(
                "%Y-%m-%d"
            ) + "-{:s}-metadata.csv".format(journal)
    ...
    print(
        "Total time to fetch and manipulate records was {:.1f} seconds.".format(
        ...
        for gene in self.gene_list:
            ...
            if self.combination_terms:
                for combination in self.combination_terms:
                    print(combination)
                    df_comb = biomedrxivsearch(
                        kwd=[str(gene) + " " + str(combination)],
                    ...
                    colname_abstract = (
                        str(gene) + "_" + str(combination) + "_" + "abstract"
                    )
                    ...
                    if not df_comb.empty:
                        ...
                        df_comb = pd.DataFrame(
                            {colname_gene: ["no_hit"], colname_abstract: ["not_hit"]}
                        )
