...
class luigi_biorxivsearch(Task):
    ...
    def output(self):
        ...
        return LocalTarget(
            "./data/{}{}_biorxiv.csv".format(str(names_gene_list), str(names_comb_list))
        )

    def run(self):
        df = biorxiv.multi_medsearch(
            gene_list=self.gene_list,
            combination_terms=self.combination_terms,
...
class luigi_pubmedsearch(Task):
    ...
    def output(self):
        ...
        names_comb_list = "_".join(self.combination_terms)
        return LocalTarget(
            "./data/{}{}_pubmed.csv".format(str(names_gene_list), str(names_comb_list))
        )
...
class combined_search(Task):
    ...
    def requires(self):
        return {
            "biorxiv": luigi_biorxivsearch(
                gene_list=self.gene_list,
                combination_terms=self.combination_terms,
                max_records=self.max_records,
            ),
            "pubmed": luigi_pubmedsearch(
                gene_list=self.gene_list,
                combination_terms=self.combination_terms,
                max_records=self.max_records,
            ),
    ...
    def output(self):
        ...
        names_comb_list = "_".join(self.combination_terms)
        return LocalTarget(
            "./data/{}{}_combined.csv".format(
                str(names_gene_list), str(names_comb_list)
            )
        )
...
class graphical_output(Task):
    ...
    def requires(self):
        if self.database == "pubmed":
            return luigi_pubmedsearch(
                gene_list=self.gene_list, combination_terms=self.combination_terms
            )
        elif self.database == "biorxiv":
            return luigi_biorxivsearch(
                gene_list=self.gene_list, combination_terms=self.combination_terms
            )

    def output(self):
        target = SuffixPreservingLocalTarget(
            "./data/{}{}_wordcloud.png".format(
                self.gene_list[0], self.combination_terms[0]
            ),
